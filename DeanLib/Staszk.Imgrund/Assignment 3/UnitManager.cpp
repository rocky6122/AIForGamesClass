...
Unit* UnitManager::createUnit(const Sprite& sprite, bool shouldWrap, const PositionData& posData /*= ZERO_POSITION_DATA*/, const PhysicsData& physicsData /*= ZERO_PHYSICS_DATA*/, const UnitID& id, bool local)
{
	Unit* pUnit = NULL;
	...
	{
		//create unit
		pUnit = new (ptr)Unit(sprite, local);//placement new
...
Unit* UnitManager::createRandomUnit(const Sprite& sprite, bool local)
{
	...
	int velX = rand() % 50 - 25;
	int velY = rand() % 40 - 20;
	Unit* pUnit = createUnit(sprite, true, PositionData(Vector2D(posX,posY),0), PhysicsData(Vector2D(velX,velY),Vector2D(0.1f,0.1f), 0.1f, 0.05f), INVALID_UNIT_ID, local);
	if (pUnit != NULL)
	{
...
void UnitManager::addUnit(Unit * unitToAdd)
{
	...
	if (ptr != NULL)
	{
		...
		/////////////////////////////////////////////////////
		//	POSSIBLY NEED TO EDIT COMPONENT SECTION BELOW  //
		/////////////////////////////////////////////////////

		//create some components
		ComponentManager* pComponentManager = gpGame->getComponentManager();
		ComponentID id = pComponentManager->allocatePositionComponent(unitToAdd->getPositionComponent()->getData(), true);
		pUnit->mPositionComponentID = id;
		pUnit->mpPositionComponent = pComponentManager->getPositionComponent(id);
		pUnit->mPhysicsComponentID = pComponentManager->allocatePhysicsComponent(pUnit->mPositionComponentID, unitToAdd->getPhysicsComponent()->getData());
		pUnit->mSteeringComponentID = pComponentManager->allocateSteeringComponent(pUnit->mPhysicsComponentID);
	}
}
