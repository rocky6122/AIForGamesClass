...
struct NetworkInfo
{
	...
	char isRunning = 0;
	//Designates when the client has recieved a packet
	char packetRecieved = 0;
	// Designates whether we are dead-reckoning
	char deadReckonThisFrame = 0;
	// Designates that we have begun data sharing
	char hasReceivedBoids = 0;

	char hasInterpolated = 0;
};

//Proccesses all the networking
void handleNetworking();

//Catches up to local time before interpolating
void catchUpReckoning(UnitManager* tempManager , float elapsedTime, float latency);

//Interpolate
void interpolation(UnitManager* tempManager);

//Dead Reckons the networked boids
void startReckoning();
