...
std::vector<Adjacency> Grid::getAdjacentIndices( int theSquareIndex ) const
{
	std::vector<Adjacency> indices;
	...
	for( int i=0; i<NUM_DIRS; i++ )
	{
		...
		if( adjX >= 0 && adjX < mGridWidth && adjY >=0 && adjY < mGridHeight )
		{
			...
			Adjacency adjacentIndex;

			adjacentIndex.index = adjIndex;

			bool diagonal = (xMods[i] != 0 && yMods[i] != 0);

			adjacentIndex.diagonal = diagonal;

			//add to vector of indices
			indices.push_back( adjacentIndex );
		}
	}
