...
void GridGraph::init()
{
	if( mpGrid != NULL )
	{
		...
		for( int i=0; i<numGridIndices; i++ )
		{
			...
			Node* pNode = new Node( i , i % mpGrid->getGridWidth(), ((int) i / (int) mpGrid->getGridWidth()));
			//add to NodeVector
			mNodes[i] = pNode;
		}
		...
		for( int i=0; i<numGridIndices; i++ )
		{
			...
			std::vector<Adjacency> adjacencies = mpGrid->getAdjacentIndices( i );
			...
			for( unsigned int adjIndex = 0; adjIndex < adjacencies.size(); adjIndex++ )
			{
				...
				if( mpGrid->getValueAtIndex(adjacencies[adjIndex].index) != BLOCKING_VALUE )
				{
					Node* pToNode = mNodes[ adjacencies[adjIndex].index ];//find to node

					float cost = (adjacencies[adjIndex].diagonal) ? 1.414f : 1.0f;
